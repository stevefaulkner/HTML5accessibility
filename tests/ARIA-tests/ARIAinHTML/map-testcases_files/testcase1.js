...
$.each(groups, function(groupName, roles ){
	...
	$.each(roles, function(roleName, roleStates) {
		...
		$.each(roleStates, function (i, state) {
			...
			html += "<div class='widgetDemo'>"+ preHTML +"<map id='role" + widgetIncrement++ +"' class='roleTest "+ roleName + dStateHTML +"' role='"+ roleName +"' "+ attribHTML + stateHTML + " aria-label='acc name'><area href='#' >&nbsp;&nbsp;&nbsp;&nbsp;</map>"+ postHTML +"</div>";
			section.append(html);
			});
