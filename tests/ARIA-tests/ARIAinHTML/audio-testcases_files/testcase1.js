...
$.each(groups, function(groupName, roles ){
	...
	$.each(roles, function(roleName, roleStates) {
		...
		$.each(roleStates, function (i, state) {
			...
			html += "<div class='widgetDemo'>"+ preHTML +"<audio controls id='role" + widgetIncrement++ +"' class='roleTest "+ roleName + dStateHTML +"' role='"+ roleName +"' "+ attribHTML + stateHTML + " aria-label='acc name'>&nbsp;&nbsp;&nbsp;&nbsp;</audio>"+ postHTML +"</div>";
			section.append(html);
			});
