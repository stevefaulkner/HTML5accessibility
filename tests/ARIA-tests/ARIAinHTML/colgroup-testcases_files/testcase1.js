...
$.each(groups, function(groupName, roles ){
	...
	$.each(roles, function(roleName, roleStates) {
		...
		$.each(roleStates, function (i, state) {
			...
			html += "<div class='widgetDemo'>"+ preHTML +"<table><colgroup id='role" + widgetIncrement++ +"' class='roleTest "+ roleName + dStateHTML +"' role='"+ roleName +"' "+ attribHTML + stateHTML + " aria-label='acc name'><col></colgroup><tr><td>&nbsp;&nbsp;&nbsp;&nbsp;</tr></table>"+ postHTML +"</div>";
			section.append(html);
			});
